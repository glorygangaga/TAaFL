...
func main:void()
{
    ...
    print(message, number, " равен ", result);
}
```
...
* Числовые литералы не содержат знака (`+` или `-`)
* Конструкции вида -5 или +3.14 состоят из двух токенов:
    * Оператор - или +
    * Числовой литерал 5 или 3.14
* **Примечание:** На уровне лексического анализа знак и число являются отдельными токенами, но на уровне синтаксического анализа унарные операторы `+` и `-` могут комбинироваться с числовыми литералами для формирования выражений
